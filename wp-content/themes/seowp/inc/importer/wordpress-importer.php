 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class lbmn_WP_Import extends WP_Importer {
	...
	//function lbmn_WP_Import() { /* nothing */ }
...
<?php
	...
	function process_menu_item( $item ) {
		...
		// Check and prepare item menu URL
		// – if WP installed in sub-folder > add it to the URL
		// – if Premalinks set to ?=id > add it to the URL

		// if ( get_option('permalink_structure') != '/%postname%/' ) {

		// }

		$menu_link_prefix = get_site_url();

		// if permalinks set to http://example.com/?p=123
		if ( get_option('permalink_structure') == '' ) {
			$menu_link_prefix = get_site_url() . '/index.php?pagename=';
		}

		if ( $_menu_item_url != '#' && $_menu_item_url !='' && !stristr($_menu_item_url, '://') ) {
			 $_menu_item_url = $menu_link_prefix . $_menu_item_url;
		}
...
// List of custo meta keys for Mega Main Menu
